...
#### 20210521(금) 작업
    - 반응형 게시판페이지(CRUD) CSS 처리: Creat(Update) = board_write.html
    - 글쓰기폼(부트스트랩 적용): 첨부파일부분, 내용입력부분 웹에디터 추가
    - 부트스트랩(Admin LTE): 제이쿼리 기반의 UI 템플릿(프레임워크)
    - AdminLTE: dost(디스트리뷰트=배포), pages(더미데이터), plugins(서머노트 등등)
    - 대시보드 파일 샘플: index.html, indext2.html, index3.html
    ...
    - 2.스프링(MVC) 웹프로젝트 만들예정. 헬로월드 1개 - 두달 간 이 프로젝트로 진행.
